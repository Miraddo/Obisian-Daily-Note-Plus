...
// src/main.ts
__export(exports, {
  default: () => DailyNotesPlugin
});
var import_obsidian2 = __toModule(require("obsidian"));

// src/settings.ts
var import_obsidian = __toModule(require("obsidian"));
var DailyNotesSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Daily Notes Folder").setDesc("Select the folder where daily notes will be created").addText((text) => {
      text.setPlaceholder("Select folder").setValue(this.plugin.settings.dailyNotesPath).onChange(async (value) => {
        this.plugin.settings.dailyNotesPath = value;
        await this.plugin.saveSettings();
        this.updatePreview();
      });
    }).addButton((button) => {
      button.setButtonText("Browse").setCta().onClick(async () => {
    ...
    new import_obsidian.Setting(containerEl).setName("Template File").setDesc("Select a template file to use for daily notes (optional)").addText((text) => {
      text.setPlaceholder("Select template file").setValue(this.plugin.settings.templatePath || "").onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Date Format").setDesc("Format for the date in filenames. Use DD for day, MM for month, YYYY for year.").addText((text) => text.setPlaceholder("DDMMYYYY").setValue(this.plugin.settings.dateFormat).onChange(async (value) => {
      this.plugin.settings.dateFormat = value;
      await this.plugin.saveSettings();
      this.updatePreview();
    }));
    new import_obsidian.Setting(containerEl).setName("Default Note Type").setDesc("Choose between markdown or canvas for new daily notes").addDropdown((dropdown) => dropdown.addOption("markdown", "Markdown").addOption("canvas", "Canvas").setValue(this.plugin.settings.defaultNoteType).onChange(async (value) => {
      this.plugin.settings.defaultNoteType = value;
      await this.plugin.saveSettings();
      this.updatePreview();
    }));
    new import_obsidian.Setting(containerEl).setName("Use YAML Frontmatter").setDesc("Add YAML frontmatter to daily notes with metadata").addToggle((toggle) => toggle.setValue(this.plugin.settings.useYamlFrontmatter).onChange(async (value) => {
...
var DailyNotesPlugin = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
  ...
  async createOrOpenDailyNote() {
    try {
      ...
      new import_obsidian2.Notice("Failed to create daily note: " + error.message);
    }
  }
  async openTodayNote() {
    try {
      ...
      if (file instanceof import_obsidian2.TFile) {
        ...
        new import_obsidian2.Notice("Today's note does not exist yet. Creating...");
        await this.createOrOpenDailyNote();
      }
    } catch (error) {
      new import_obsidian2.Notice("Failed to open today's note: " + error.message);
    }
  }
    ...
    try {
      if (file.extension === "md") {
        ...
        if (matches) {
          ...
          const dateFormat = this.settings.dateFormat.replace("DD", today.getDate().toString().padStart(2, "0")).replace("MM", (today.getMonth() + 1).toString().padStart(2, "0")).replace("YYYY", today.getFullYear().toString());
          ...
          const { filePath: filePath2 } = this.getDailyNotePath();
          let dailyFile2 = this.app.vault.getAbstractFileByPath(filePath2);
          if (!(dailyFile2 instanceof import_obsidian2.TFile)) {
            const content2 = await this.getTemplateContent();
            const newFile = await this.app.vault.create(filePath2, content2);
            if (newFile instanceof import_obsidian2.TFile) {
              await this.app.workspace.getLeaf().openFile(newFile);
            }
          } else {
            await this.app.workspace.getLeaf().openFile(dailyFile2);
          }
          return;
        }
      }
      ...
      new import_obsidian2.Notice("Error handling daily note: " + error.message);
    }
  }
  getDailyNotePath() {
    const today = new Date();
  ...
  async getTemplateContent() {
    const date = (0, import_moment.default)().format("MMMM D, YYYY");
...
date: ${date}
type: daily-note
created: ${(0, import_moment.default)().format()}
---
